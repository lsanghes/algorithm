...
tank has 2 units of gas. You need cost[1] = 1 gas to get to station 0. So, you
travel to station 0 and still have 1 unit of gas left over. You fill in
...
'''
class Solution:
    # Valid starting station exist if total gas is more than total cost
    def canCompleteCircuit(self, gas, cost):
        cur_tank = start = 0
        for i in range(len(gas)):
            cur_tank += gas[i] - cost[i]
            if cur_tank < 0:
                # start cannot make it to i
                # no need to try anything between [start, i]
                start = i + 1
                cur_tank = 0
        return start if sum(gas)-sum(cost) >= 0 else -1

# test
print(Solution().canCompleteCircuit([1,2,7,2,1], [4,3,3,2,1]))
print(Solution().canCompleteCircuit([1,2,6,2,1], [4,3,3,2,1]))
